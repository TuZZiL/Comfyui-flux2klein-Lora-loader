...
| `anatomy_profile` | Додатковий overlay для збереження тіла при редагуванні одягу або тіла. Він накладається поверх `edit_mode`. Залишайте вимкненим, доки він не знадобиться. |

## Дві осі захисту

`edit_mode` і `anatomy_profile` вирішують різні задачі:

- `edit_mode` = семантичний / identity-захист. Він керує тим, наскільки сильно LoRA може переписувати суб'єкт, обличчя та prompt-driven зміст.
- `anatomy_profile` = захист тіла / силуету. Він додає окремий shield для структури, коли clothing/body LoRA змінює пропорції сильніше, ніж потрібно.

Вони спеціально можуть працювати разом. Коли ви вмикаєте обидва, їхні множники комбінуються, тому фінальний захист стає жорсткішим, ніж у кожної системи окремо.

Практичне правило:
- Починайте лише з `edit_mode`.
- Додавайте `anatomy_profile` тільки якщо тіло, силует або пропорції все ще пливуть.
...
| **Raw** | Нічого | "Сира" LoRA, повна свобода (`None` лишається legacy-аліасом) |
...
| `strength` | float | Глобальна сила LoRA (-3.0 до 3.0) |
...
**Практичне правило:** `edit_mode` це шар семантичного / identity-захисту, а `anatomy_profile` це overlay для захисту тіла / структури. Використовуйте обидва лише тоді, коли вам справді потрібні обидва типи захисту.
...
Слот 3: enhancer LoRA      → edit_mode=Raw, strength=0.2–0.4
```
...
| `strength` | float | Базова сила LoRA (-3.0–3.0) |
...
> **Важливо:** `recommended_edit_mode=Raw` означає "Raw / No Protection", а не "нічого не обрано" (`None` все ще приймається як legacy alias).
...
- Auto вибрав не той пресет: переключіться з `Auto` на `Preserve Face` або `Raw`. Auto робить найкращу здогадку, але не читає ваш намір.
...
- **Рівномірний** (enhancer-и, слайдери) → **Raw**
...
**Q: Що означає `Raw` в edit_mode?**
A: "Raw / No Protection". LoRA працює з усіма шарами на рівній силі. Legacy-значення `None` теж підтримується.
...
A: Auto не читає ваш намір. Переключіться на ручний: `Preserve Face` для identity-роботи, `Raw` для повної свободи. Підлаштовуйте `protection` як діал захисту (`balance` лишився legacy-аліасом).
